<!DOCTYPE html>
<html>

<head>
    <title>Register to PMPML Bus Ticket</title>
    <link rel="stylesheet" href="style/style.css">
    <script src="style/javascriptfile.js"></script>
    <script src="https://kit.fontawesome.com/ea215485af.js" crossorigin="anonymous"></script>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <script>
        if (window.history.replaceState) {
            window.history.replaceState(null, null, window.location.href);
        }
    </script>
</head>

<body>
    <div class="header">
        <div class="logo"><img src="images/app.png"></div>
        <div class="name">Pune Mahanagar Parivahan Mahamandal Limited</div>
        <div class="sname">PMPML</div>
        <div class="logo"><img src="images/pmpml.jfif"></div>

    </div>
    <div class="nav" id="nav">
        <a href="home.html"><i class="fa fa-home" style="padding-right:10px"></i>Home</a>
        <a href="about.html"><i class="fa fa-info" style="padding-right:10px"></i>About us</a>
        <a href="help.html"><i class="fas fa-question" style="padding-right:10px"></i>Help</a>
        <a href="feedback.php"><i class="fas fa-comment-dots" style="padding-right:10px"></i>Feedback</a>
        <a href="javascript:void(0);" class="icon" onclick="mynavbar()"><i class="fa fa-bars"></i></a>
    </div>
    <div class="loginbody" style="background:url(images/bg.jpeg);background-repeat: no-repeat;background-size:cover;background-position: center;">
        <center>
            <form name="register" method="POST" action="<?php echo $_SERVER['PHP_SELF']; ?>">
                <table cellpadding="10px">
                    <tr>
                        <th colspan="2">
                            <text class="title">REGISTER</text>
                        </th>
                    </tr>
                    <tr>
                        <th><i class="fas fa-user fa-lg" style="padding-right:10px"></i>Enter Your Name: -</th>
                        <td><input type="text" name="uname" id="uname"></td>
                    </tr>
                    <tr>
                        <th><i class="fas fa-phone-alt fa-lg" style="padding-right:10px"></i>Enter Your Number: -</th>
                        <td><input type="text" name="unumber" id="unumber"></td>
                    </tr>
                    <tr>
                        <th><i class="far fa-envelope fa-lg" style="padding-right:10px"></i>Enter Your Email: -</th>
                        <td><input type="text" name="uemail" id="uemail"></td>
                    </tr>
                    <tr>
                        <th><i class="fas fa-unlock-alt fa-lg" style="padding-right:10px"></i>Enter Password</th>
                        <td><input type="password" name="pass" id="pass1"></td>
                    </tr>
                    <tr>
                        <th><i class="fas fa-lock fa-lg" style="padding-right:10px"></i>Confirm Password</th>
                        <td><input type="password" name="pass" id="pass2"></td>
                    </tr>
                    <tr>
                        <td colspan="2"><input type="checkbox" onclick="showpassword()">Show Password</td>
                    </tr>
                    <tr>
                        <td colspan="2"><input type="submit" name="register" value="Register" onclick=" return rvalidate();"></td>
                    </tr>
                    <tr>
                        <td colspan="2">if have an account <a href="login.php">Login</a></td>
                    </tr>
                </table>
            </form>
        </center>
    </div>
    <div class="footer">
        <div class="follow">
            <font size="5px">Follow us on</font>
            <a href=""><i class="fab fa-facebook-square fa-2x"></i></a>
            <a href=""><i class="fab fa-instagram-square fa-2x"></i></a>
            <a href=""><i class="fab fa-twitter-square fa-2x"></i></a>
        </div>
        <div class="cr">Copyright &copy 2020 OBTPMPML All Rights Reserved.</div>
    </div>
</body>

</html>
<?php
if ($_SERVER['REQUEST_METHOD'] === "POST") {
    error_reporting(0);
    include_once('connection.php');
    $name = $_POST['uname'];
    $number = $_POST['unumber'];
    $email = $_POST['uemail'];
    $pass = $_POST['pass'];

    $query1 = "SELECT unumber from users_data where unumber='$number'";
    $result1 = mysqli_query($conn, $query1);
    $rows_n = mysqli_fetch_assoc($result1);
    $query2 = "SELECT uemail from users_data where uemail='$email'";
    $result2 = mysqli_query($conn, $query2);
    $rows_e = mysqli_fetch_assoc($result2);
    if ($rows_n['unumber'] == $number) {
        echo "<script type=text/javascript> alert('You have Register before with this Number " . $rows_n['unumber'] . ".')</script>";
    } else if ($rows_e['uemail'] == $email) {
        echo "<script type=text/javascript> alert('You have Register before with this Email " . $rows_e['uemail'] . ".')</script>";
    } else if (mysqli_num_rows($result1) == 0 && mysqli_num_rows($result2) == 0) {
        $query = "INSERT INTO users_data (uname, unumber,uemail,upassword) VALUES ('$name', '$number', '$email', '$pass');";
        if (mysqli_query($conn, $query)) {
            echo "<script type=text/javascript>
            x=confirm('Successfully Registered...Click ok to login.');
            if(x==true){ window.location='login.php';
            }</script>";
        }
    }
}
?>